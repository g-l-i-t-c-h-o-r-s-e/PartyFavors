...
FFMPREG -ss $seek -i $1 -f nut -s $2 -vcodec huffyuv -acodec pcm_u32le \
...
-map "[out]" -map "[out2]" -t $add - | FFMPREG -i - -f nut -c:v huffyuv -c:a pcm_u32le -y ./results/out_$zeropad.nut
...
cd ./results
 ...
 wait
  ...
  FFMPREG -f concat -safe 0 -i ieatass.txt -codec copy -y KMS.nut
 FFPLEY KMS.nut -af volume=0.1 ; fi
